...
import { createOrder, getCategories, getProducts } from "../api";
...
const StorePage = () => {
  ...
  useEffect(() => { const sync = async () => { try { const [nextProducts, nextCategories] = await Promise.all([getProducts(), getCategories()]); setProducts(nextProducts); setCategories(nextCategories.length ? nextCategories : defaultCategories); write(PRODUCT_KEY, nextProducts); write(CATEGORY_KEY, nextCategories); } catch { /* keep the last cached data while offline */ } }; sync(); const timer = setInterval(sync, 3000); window.addEventListener("focus", sync); return () => { clearInterval(timer); window.removeEventListener("focus", sync); }; }, []);
  ...
  const placeOrder = async (customer) => { if (!cart.length) return false; setOrderLoading(true); const items = cart.map((item) => ({ product: item._id, name: item.name, price: Number(item.price), qty: item.qty, image: item.images?.[0] || item.image || "", size: item.size || "", color: item.color || "" })); const order = { customer, items, total: items.reduce((sum, item) => sum + item.price * item.qty, 0), status: "Pending" }; try { const saved = await createOrder(order); setCart([]); setCheckoutOpen(false); setMessage(`Order ${saved.orderNumber || ""} placed successfully.`); setOrderLoading(false); return true; } catch (error) { setMessage(error.message || "Order save nahi hua. Internet/API check karein."); setOrderLoading(false); return false; } };
